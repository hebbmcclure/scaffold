...
public class BaseWebElement {
     ...
     */
    @Getter protected WebElementWait webElementWait;
     ...
     * {@link By#cssSelector(String)} over {@link By#xpath(String)} in almost all cases as it can be less flaky and
     * less reliant on DOM hierarchy.
     *
    ...
    public String toString() {
        ...
        //If there's a parentBy or a parentElement, start with their toString() method. There should only ever be
        // one or the other
        if (parentBy != null) {
            toString = String.format("Parent By: [%s]", parentBy);
        }

        //If there's no by locator, there should be a baseElement. If that's all we've got, we're no worse off than
        // we were with webelement.toString()
        if (null != by) {
            toString += String.format("By: [%s]", by);
        }
    ...
    /**
     * A helper method for {@link #findElements(Class, By)} and {@link #findElements(Class, By)}.
     *
     * @param elementClass  the class of the element being wrapped
     * @param by            the child by locator
     * @param combinedBy    the combined by locator
     * @param parentBy      the parent by locator
     * @param <T>           the type reference that extends {@link BaseWebElement}
     * @return              as element of T
     */
    private <T extends BaseWebElement> T getScaffoldElement(Class<T> elementClass, By by, By combinedBy, By parentBy) {
        T newElement;
        try {
